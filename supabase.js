...
const DB = {
  ...
  /* ── USUARIOS ────────────────────────────────── */
  async getUsuarios() {
    const { data, error } = await sb.from(CONFIG.TABLA_USUARIOS).select('*').eq('activo', 1).order('created_at');
    return sbCheck(data, error, 'getUsuarios');
  },

  async loginUsuario(username, password) {
    const { data, error } = await sb.from(CONFIG.TABLA_USUARIOS).select('*')
      .eq('username', username).eq('password', password).eq('activo', 1).maybeSingle();
    return sbCheck(data, error, 'loginUsuario');
  },

  async crearUsuario(u) {
    const { data, error } = await sb.from(CONFIG.TABLA_USUARIOS).insert([u]).select().single();
    return sbCheck(data, error, 'crearUsuario');
  },

  async actualizarUsuario(id, cambios) {
    cambios.updated_at = new Date().toISOString();
    const { data, error } = await sb.from(CONFIG.TABLA_USUARIOS).update(cambios).eq('id', id).select().single();
    return sbCheck(data, error, 'actualizarUsuario');
  },

  async desactivarUsuario(id) {
    const { error } = await sb.from(CONFIG.TABLA_USUARIOS)
      .update({ activo: 0, updated_at: new Date().toISOString() }).eq('id', id);
  ...
  async getIncidencia(id) {
    const { data, error } = await sb.from(CONFIG.TABLA_INCIDENCIAS).select('*').eq('id', id).single();
    return sbCheck(data, error, 'getIncidencia');
  },

  async crearIncidencia(inc) {
    const { data, error } = await sb.from(CONFIG.TABLA_INCIDENCIAS).insert([inc]).select().single();
  ...
  async actualizarIncidencia(id, cambios) {
    ...
    const { data, error } = await sb.from(CONFIG.TABLA_INCIDENCIAS).update(cambios).eq('id', id).select().single();
    return sbCheck(data, error, 'actualizarIncidencia');
  },

  async eliminarIncidencia(id) {
    const { error } = await sb.from(CONFIG.TABLA_INCIDENCIAS).delete().eq('id', id);
    sbCheck(null, error, 'eliminarIncidencia');
  },

  /* ── LOG ─────────────────────────────────────── */
  async getLog() {
    const { data, error } = await sb.from(CONFIG.TABLA_LOG).select('*').order('created_at', { ascending: false });
    return sbCheck(data, error, 'getLog');
  },

  async escribirLog(entrada) {
    const { error } = await sb.from(CONFIG.TABLA_LOG).insert([entrada]);
    sbCheck(null, error, 'escribirLog');
  },

  /* ── DASHBOARD SNAPSHOT ──────────────────────── */
  async getDashboard() {
    const { data, error } = await sb.from(CONFIG.TABLA_SNAPSHOT).select('*').eq('id', 1).single();
    return sbCheck(data, error, 'getDashboard');
  },

  async guardarDashboard(datos, usuario) {
    const { error } = await sb.from(CONFIG.TABLA_SNAPSHOT)
  ...
  async getCinesUnicos() {
    const { data, error } = await sb.from('cp_maquinas').select('cine').order('cine');
